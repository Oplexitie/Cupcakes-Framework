...
export var pan_speed : Vector2 = Vector2(5,2)
export var office_clamp : Vector2 = Vector2(500,0.3)
...
func _ready():
	...
	var border_correcter_x : float = 1/(OS.get_window_size().x/view_size.x)
	var border_correcter_y : float = 1/(OS.get_window_size().y/view_size.y)
